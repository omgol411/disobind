...
###################################################################################
	...
	def download_uniprot_seq( self, headers ):
		...
		uniprot_list = []
		for head in headers:
			uniprot1 = head.split( ":" )[0]
			uniprot2 = head.split( "--" )[1].split( ":" )[0]
			uniprot_list.append( uniprot1 )
			uniprot_list.append( uniprot2 )
		uniprot_list = list( set( uniprot_list ) )
		representative_headers = [f"{uni_id}:1:2--{uni_id}:1:2_0" for uni_id in uniprot_list]
		with Pool( self.cores ) as p:
			for result in tqdm.tqdm( p.imap_unordered( self.parallelize_uni_seq_download, representative_headers ), total = len( representative_headers ) ):
